...
public class Add extends Operation<Matrix> {

    public Add(Node<Matrix> node1, Node<Matrix> node2) {
        super(node1, node2);
    }

    @Override
    public Matrix compute() {
        return inputNodes.get(0).getOutput().plus(inputNodes.get(1).getOutput());
    }
    ...
    public List<Matrix> computeDownstreamGradients(Matrix upstreamGradient) {
        ...
        for(int i = 0; i < dataSecond.length; i++) {
            for (int j = 0; j < dataSecond[i].length; j++) {
                dataSecond[i][j] = upstreamGradient.getData()[i][j];
            }
        }
        return result;
    }
